...
function mapChartOptions(value: unknown): ChartOptions {
	...
	return {
		centralLabel: options.centralLabel
			? String(options.centralLabel)
			: undefined,
		centralSubLabel: options.centralSubLabel
			? String(options.centralSubLabel)
			: undefined,
	};
}
...
function mapWidgetFromRow(row: Record<string, unknown>): DashboardWidget {
	return {
		...
		numericValue:
			row.numeric_value === null ? undefined : Number(row.numeric_value ?? 0),
...
export function useDashboardStore(coupleSlug?: string) {
	...
	const couple = computed(() =>
		state.value.couples.find((item) => item.slug === coupleSlug),
	);
	...
	const visibleWidgets = computed(() =>
		widgets.value.filter((item) => item.visible),
	);
	const alerts = computed(() =>
		state.value.alerts
			.filter(
				(item) => item.coupleId === couple.value?.id && isAlertVisible(item),
			)
	...
	async function triggerAlert(
		alert: Omit<CoupleAlert, "id" | "createdAt" | "active">,
	) {
